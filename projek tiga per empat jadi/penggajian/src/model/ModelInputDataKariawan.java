...
public class ModelInputDataKariawan {
    ...
    public void deleteKariawan(String dataterpilih) {
        try{//SELECT * FROM `karyawan` JOIN `divisi` WHERE karyawan.id_divisi = divisi.id
    ...
    public void updateKariawan(String Dataterpilih, String nama, String alamat, String no,  String ps, String dv) {
        try {
            String id_dv = getIddevisi(dv);
            /*if(nama.equals(null) ){
            String sql = "SELECT * FROM `karyawan` WHERE nik = '"+Dataterpilih+"'";
            ResultSet hasil = statement.executeQuery(sql);
            nama = hasil.getString("nama");
            }
            if(alamat.equals(null)null ){
            String sql = "SELECT * FROM `karyawan` WHERE nik = '"+Dataterpilih+"'";
            ResultSet hasil = statement.executeQuery(sql);
        ...
        /*
        if(no == null ){
        no = "SELECT `no_hp` FROM `karyawan` WHERE nik = '"+Dataterpilih+"'";
        }
        /*if(dv == null ){
        id_dv = "SELECT `id_divisi` FROM `karyawan` WHERE nik = '"+nik+"'";
        }
        if(ps == null ){
            ps = "SELECT `password` FROM `karyawan` WHERE nik = '"+Dataterpilih+"'";
        }*/   
